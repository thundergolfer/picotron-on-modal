...
class ProcessGroupManager:
    def __init__(self, dp_size, pp_size, tp_size):
        ...
        ), f"World size ({self.world_size}) != DP ({self.dp_size}) * PP ({self.pp_size}) * TP ({self.tp_size})"
        ...
        self.dp_group = dist.new_subgroups_by_enumeration(
            [
                self.grid[:, p, t].tolist()
                for p in range(pp_size)
                for t in range(tp_size)
            ]
        )[0]
        self.pp_dp_group = dist.new_subgroups_by_enumeration(
            [self.grid[:, :, t].flatten().tolist() for t in range(tp_size)]
        )[0]
        ...
        self.pp_next_rank = (
            ...
            else int(self.grid[self.dp_rank, self.pp_rank + 1, self.tp_rank].item())
        )
        self.pp_prev_rank = (
            None
            if self.pp_rank == 0
