...
class Reduce(torch.autograd.Function):
    ...
    def forward(ctx, input):
        if pgm.process_group_manager.tp_world_size == 1:
            return input
        dist.all_reduce(
            input, op=dist.ReduceOp.SUM, group=pgm.process_group_manager.tp_group
        )
        return input
...
class Copy(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        if pgm.process_group_manager.tp_world_size == 1:
            return grad_output
        dist.all_reduce(
            grad_output, op=dist.ReduceOp.SUM, group=pgm.process_group_manager.tp_group
        )
        return grad_output
...
class VocabParallelEmbedding(nn.Module):
    ...
    def _vocab_range_from_global_vocab_size(
        ...
        ), f"{global_vocab_size} is not divisible by {world_size}"
